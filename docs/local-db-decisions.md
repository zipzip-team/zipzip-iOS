...
## 결정 9. `device.is_registered` — 사용자 등록 기기로 사진 필터링

**결정**: `device`에 `is_registered`(0/1) 컬럼을 두고, 온보딩·마이페이지에서 사용자가 선택한 기기만 등록. 사진뷰와 필터 후보(기기 축)는 등록된 기기로 제한한다.

**근거**
- 사용자는 자기 기기로 찍은 사진만 모아보길 원함 → "볼 기기"를 영속 저장하는 축이 필요(온보딩 선택은 로컬 상태만으로는 유실됨).
- 별도 `registered_device` 테이블 대신 `device`의 불리언 컬럼으로 두면 사진 필터를 `photo JOIN device ... WHERE is_registered`로 단순 처리 가능. 기기 수가 적어 별도 인덱스는 불필요.
- 등록 세트 교체는 "전체 `false` → 선택 `true`"(2 UPDATE)로 처리. 온보딩 저장·마이페이지 등록/제거가 모두 이 경로를 공유.
- 기기 분류(아이폰/갤럭시/디지털 카메라 등)는 `device.make/model`을 번들 리소스(`DeviceCatalog.json`) 기반 `DeviceModelCatalog`로 판정하며, 갤럭시(안드로이드 폰)는 등록 대상 UI에서 제외한다.

## 결정 10. 즐겨찾기 앨범 — `album.is_favorite` 플래그 + `album_photo` 소속

**결정**: 즐겨찾기를 `photo.is_favorite`(PhotoKit 하트 미러)가 아니라, `album.is_favorite = 1`로 표시한 **시스템 앨범**으로 구현한다. 사진의 즐겨찾기 소속은 일반 앨범과 동일하게 `album_photo` 행으로 관리한다.

**근거**
- 요구사항이 "앨범뷰 첫 번째에 위치하는, 삭제 불가한 즐겨찾기 **앨범**" → 사진 그리드의 앨범 엔티티가 필요. 단일 불리언 `photo.is_favorite`로는 앨범 카드·썸네일·카운트를 재사용할 수 없다.
- `album_photo` 재사용으로 담기/빼기·썸네일 3개·카운트 로직을 **기존 앨범 경로 그대로** 사용.
- 마이그레이션으로 1행 시드 → 최초 실행부터 존재 보장(별도 first-launch 코드 불필요).
- 특수 규칙은 플래그로 분기: 정렬 `is_favorite DESC` 선두 고정, 삭제 가드, **0장이면 목록에서 숨김**(빈↔비어있지 않음 경계에서만 그리드에 등장/소멸).

**`photo.is_favorite`와의 관계**: 둘은 **독립**이다. `photo.is_favorite`는 iOS 사진 앱 하트의 읽기 전용 미러(동기화 시 최신화, [결정 6](#결정-6-db-저장-여부-판단-기준) 참고)이고, 즐겨찾기 앨범은 앱 내 큐레이션이다. 현재 서로 반영하지 않는다.

**대안 (`album.sort_order`와의 차이)**: 선두 고정은 사용자 수동 정렬이 아니라 시스템 플래그 기반이므로 [결정 8](#결정-8-mvp-컬럼-확정--파생미사용-컬럼-제거)의 `album.sort_order` 제거와 상충하지 않는다.

---

## 미결 / 추후 결정

- 즐겨찾기 앨범(`album.is_favorite`) ↔︎ `photo.is_favorite`/PhotoKit 하트 연동 여부 — 현재 독립. 별 버튼이 사진 앱 하트에도 반영되길 원하면 연동 규칙 필요
