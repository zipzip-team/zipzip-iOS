...
| id (PK) | name | created_at | is_favorite |
| --- | --- | --- | --- |
| 1 | 즐겨찾기 | 2026-07-01 | 1 |
| 2 | 여름 여행 | 2026-07-02 | 0 |

> **즐겨찾기 앨범**(`is_favorite = 1`)은 앱 첫 실행 때 마이그레이션으로 **자동 시드**되는 특수 앨범이다. 사진 상세뷰의 **별 버튼**이 이 앨범에 사진을 담고/빼는 토글로 동작하며, 담김은 다른 앨범과 똑같이 `album_photo` 한 줄로 기록된다. 규칙: **삭제 불가 · 목록 맨 앞 고정 · 담긴 사진 0장이면 목록에서 숨김**. (사진 앱 하트를 미러링하는 `photo.is_favorite`와는 별개 개념 — 시나리오 5 참고.)
...
## 시나리오 5. 즐겨찾기 토글 (사진 앱 하트 = `photo.is_favorite`)

> ⚠️ **두 가지 "즐겨찾기"를 구분**하자. 이 시나리오는 **iOS 사진 앱의 하트**(`photo.is_favorite`)를 미러링하는 축이다. 사진 상세뷰의 별 버튼이 담는 **'즐겨찾기 앨범'**(시나리오 2)은 이와 **별개**로, `album_photo` 소속으로 관리되며 PhotoKit·`photo.is_favorite`와 동기화되지 않는다.
...
| 로컬 원본 | `album` | 사진집 (`is_favorite=1`인 시스템 '즐겨찾기' 앨범 1개 시드 포함) |
| 연결 | `album_photo` | 앨범 ↔︎ 사진 (다대다), 담은 시각 기록 (즐겨찾기 앨범 소속도 이 표) |
